...
(
  ...
  ,NCOD_ASEGURADO
  ,SNOMBRE_ASEG
  ,SAPELLIDO_PATERNO_ASEG
  ,SAPELLIDO_MATERNO_ASEG
  ,SAPELLIDO_CASADA_ASEG
  ,SESTADO_ASEG
  ,SGENERO_ASEG
  ,SRELACION_ASEG
  ,STIPO_PERSONA_ASEG 
...
--        ,SUBSTR(UPPER(TRIM(vpol.tipopoliza)),1,99) As TIPOPOLIZA
       ...
       ,SUBSTR(UPPER(TRIM(vpol.RegionalPoliza)),1,199) REGIONALPOLIZA
        ...
        ,SUBSTR(UPPER(TRIM(REGEXP_REPLACE(aseg.SLASTNAME, '[.,-]'))), 1, 840)As ApellidoPaterno
        ,SUBSTR(UPPER(TRIM(REGEXP_REPLACE(aseg.SLASTNAME2, '[.,-]'))), 1, 840)As ApellidoMaterno
        ,SUBSTR(UPPER(TRIM(REGEXP_REPLACE(aseg.SMARRIEDSURNAME, '[.,-]'))), 1, 840)As ApellidoCasada

SELECT       
            ...
            , to_date(Pol.DISSUEDAT,'DD-MM-YYYY') As FechaEmision
            , to_date(pol.DSTARTDATE,'DD-MM-YYYY') As InicioVigenciaPoliza
            , to_date(pol.DEXPIRDAT,'DD-MM-YYYY') as FinVigenciaPoliza
           ...
           CROSS APPLY (                                    
            ...
            CROSS APPLY (
                   SELECT NVL(xpre.npercent,0) Porcentaje FROM DISC_XPREM xpre
                   ...
                   WHERE XPre.npolicy=pol.npolicy and XPre.scertype= pol.scertype  and XPre.nbranch= pol.nbranch and XPre.nproduct=pol.nproduct
            ...
            --LEFT JOIN COMMISS_PR Comi ON Cert.SCERTYPE=Comi.SCERTYPE AND Cert.NBRANCH=Comi.NBRANCH AND Cert.NPRODUCT=Comi.NPRODUCT 
            -- AND Cert.NRECEIPT=Comi.NRECEIPT AND Cert.NDIGIT=Comi.NDIGIT AND Cert.NPAYNUMBE=Comi.NPAYNUMBE AND Cert.NINTERMED=Comi.NINTERMED 
                        
            WHERE cert.ncertif=0 
             --and Pol.NPolicy   IN (1569)
             and Pol.SCERTYPE =   '2' -- Poliza
             
             --AND PRE.SSTATUSVA    IN   ('6')--Terminada/Anulada  
